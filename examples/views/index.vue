<template>
    <Row style="margin-left: -8px; margin-right: -8px;">
        <Col v-for="x in iconList" span="3" style="padding-left: 8px;text-align:center; padding-right: 8px;">
            <Icon type={x.type} style="font-size: 32px;"></Icon>
            <div style="color: #666;opacity: .7;">ionic</div>
        </Col>
    </Row>
</template>
<script>
export default {
  data(){
      return {
         iconList:[
             {type:"ionic"},

{type:"arrow-up-a"},

{type:"arrow-right-a"},

{type:"arrow-down-a"},

{type:"arrow-left-a"},

{type:"arrow-up-b"},

{type:"arrow-right-b"},

{type:"arrow-down-b"},

{type:"arrow-left-b"},

{type:"arrow-up-c"},

{type:"arrow-right-c"},

{type:"arrow-down-c"},

{type:"arrow-left-c"},

{type:"arrow-return-right"},

{type:"arrow-return-left"},

{type:"arrow-swap"},

{type:"arrow-shrink"},

{type:"arrow-expand"},

{type:"arrow-move"},

{type:"arrow-resize"},

{type:"chevron-up"},

{type:"chevron-right"},

{type:"chevron-down"},

{type:"chevron-left"},

{type:"navicon-round"},

{type:"navicon"},

{type:"drag"},

{type:"log-in"},

{type:"log-out"},

{type:"checkmark-round"},

{type:"checkmark"},

{type:"checkmark-circled"},

{type:"close-round"},

{type:"close"},

{type:"close-circled"},

{type:"plus-round"},

{type:"plus"},

{type:"plus-circled"},

{type:"minus-round"},

{type:"minus"},

{type:"minus-circled"},

{type:"information"},

{type:"information-circled"},

{type:"help"},

{type:"help-circled"},

{type:"backspace-outline"},

{type:"backspace"},

{type:"help-buoy"},

{type:"asterisk"},

{type:"alert"},

{type:"alert-circled"},

{type:"refresh"},

{type:"loop"},

{type:"shuffle"},

{type:"home"},

{type:"search"},

{type:"flag"},

{type:"star"},

{type:"heart"},

{type:"heart-broken"},

{type:"gear-a"},

{type:"gear-b"},

{type:"toggle-filled"},

{type:"toggle"},

{type:"settings"},

{type:"wrench"},

{type:"hammer"},

{type:"edit"},

{type:"trash-a"},

{type:"trash-b"},

{type:"document"},

{type:"document-text"},

{type:"clipboard"},

{type:"scissors"},

{type:"funnel"},

{type:"bookmark"},

{type:"email"},

{type:"email-unread"},

{type:"folder"},

{type:"filing"},

{type:"archive"},

{type:"reply"},

{type:"reply-all"},

{type:"forward"},

{type:"share"},

{type:"paper-airplane"},

{type:"link"},

{type:"paperclip"},

{type:"compose"},

{type:"briefcase"},

{type:"medkit"},

{type:"at"},

{type:"pound"},

{type:"quote"},

{type:"cloud"},

{type:"upload"},

{type:"more"},

{type:"grid"},

{type:"calendar"},

{type:"clock"},

{type:"compass"},

{type:"pinpoint"},

{type:"pin"},

{type:"navigate"},

{type:"location"},

{type:"map"},

{type:"lock-combination"},

{type:"locked"},

{type:"unlocked"},

{type:"key"},

{type:"arrow-graph-up-right"},

{type:"arrow-graph-down-right"},

{type:"arrow-graph-up-left"},

{type:"arrow-graph-down-left"},

{type:"stats-bars"},

{type:"connection-bars"},

{type:"pie-graph"},

{type:"chatbubble"},

{type:"chatbubble-working"},

{type:"chatbubbles"},

{type:"chatbox"},

{type:"chatbox-working"},

{type:"chatboxes"},

{type:"person"},

{type:"person-add"},

{type:"person-stalker"},

{type:"woman"},

{type:"man"},

{type:"female"},

{type:"male"},

{type:"transgender"},

{type:"fork"},

{type:"knife"},

{type:"spoon"},

{type:"soup-can-outline"},

{type:"soup-can"},

{type:"beer"},

{type:"wineglass"},

{type:"coffee"},

{type:"icecream"},

{type:"pizza"},

{type:"power"},

{type:"mouse"},

{type:"battery-full"},

{type:"battery-half"},

{type:"battery-low"},

{type:"battery-empty"},

{type:"battery-charging"},

{type:"wifi"},

{type:"bluetooth"},

{type:"calculator"},

{type:"camera"},

{type:"eye"},

{type:"eye-disabled"},

{type:"flash"},

{type:"flash-off"},

{type:"qr-scanner"},

{type:"image"},

{type:"images"},

{type:"wand"},

{type:"contrast"},

{type:"aperture"},

{type:"crop"},

{type:"easel"},

{type:"paintbrush"},

{type:"paintbucket"},

{type:"monitor"},

{type:"laptop"},

{type:"ipad"},

{type:"iphone"},

{type:"ipod"},

{type:"printer"},

{type:"usb"},

{type:"outlet"},

{type:"bug"},

{type:"code"},

{type:"code-working"},

{type:"code-download"},

{type:"fork-repo"},

{type:"network"},

{type:"pull-request"},

{type:"merge"},

{type:"xbox"},

{type:"playstation"},

{type:"steam"},

{type:"closed-captioning"},

{type:"videocamera"},

{type:"film-marker"},

{type:"disc"},

{type:"headphone"},

{type:"music-note"},

{type:"radio-waves"},

{type:"speakerphone"},

{type:"mic-a"},

{type:"mic-b"},

{type:"mic-c"},

{type:"volume-high"},

{type:"volume-medium"},

{type:"volume-low"},

{type:"volume-mute"},

{type:"levels"},

{type:"play"},

{type:"pause"},

{type:"stop"},

{type:"record"},

{type:"skip-forward"},

{type:"skip-backward"},

{type:"eject"},

{type:"bag"},

{type:"card"},

{type:"cash"},

{type:"pricetag"},

{type:"pricetags"},

{type:"thumbsup"},

{type:"thumbsdown"},

{type:"happy-outline"},

{type:"happy"},

{type:"sad-outline"},

{type:"sad"},

{type:"bowtie"},

{type:"tshirt-outline"},

{type:"tshirt"},

{type:"trophy"},

{type:"podium"},

{type:"ribbon-a"},

{type:"ribbon-b"},

{type:"university"},

{type:"magnet"},

{type:"beaker"},

{type:"erlenmeyer-flask"},

{type:"egg"},

{type:"earth"},

{type:"planet"},

{type:"lightbulb"},

{type:"cube"},

{type:"leaf"},

{type:"waterdrop"},

{type:"flame"},

{type:"fireball"},

{type:"bonfire"},

{type:"umbrella"},

{type:"nuclear"},

{type:"no-smoking"},

{type:"thermometer"},

{type:"speedometer"},

{type:"model-s"},

{type:"plane"},

{type:"jet"},

{type:"load-a"},

{type:"load-b"},

{type:"load-c"},

{type:"load-d"},

{type:"ios-ionic-outline"},

{type:"ios-arrow-back"},

{type:"ios-arrow-forward"},

{type:"ios-arrow-up"},

{type:"ios-arrow-right"},

{type:"ios-arrow-down"},

{type:"ios-arrow-left"},

{type:"ios-arrow-thin-up"},

{type:"ios-arrow-thin-right"},

{type:"ios-arrow-thin-down"},

{type:"ios-arrow-thin-left"},

{type:"ios-circle-filled"},

{type:"ios-circle-outline"},

{type:"ios-checkmark-empty"},

{type:"ios-checkmark-outline"},

{type:"ios-checkmark"},

{type:"ios-plus-empty"},

{type:"ios-plus-outline"},

{type:"ios-plus"},

{type:"ios-close-empty"},

{type:"ios-close-outline"},

{type:"ios-close"},

{type:"ios-minus-empty"},

{type:"ios-minus-outline"},

{type:"ios-minus"},

{type:"ios-information-empty"},

{type:"ios-information-outline"},

{type:"ios-information"},

{type:"ios-help-empty"},

{type:"ios-help-outline"},

{type:"ios-help"},

{type:"ios-search"},

{type:"ios-search-strong"},

{type:"ios-star"},

{type:"ios-star-half"},

{type:"ios-star-outline"},

{type:"ios-heart"},

{type:"ios-heart-outline"},

{type:"ios-more"},

{type:"ios-more-outline"},

{type:"ios-home"},

{type:"ios-home-outline"},

{type:"ios-cloud"},

{type:"ios-cloud-outline"},

{type:"ios-cloud-upload"},

{type:"ios-cloud-upload-outline"},

{type:"ios-cloud-download"},

{type:"ios-cloud-download-outline"},

{type:"ios-upload"},

{type:"ios-upload-outline"},

{type:"ios-download"},

{type:"ios-download-outline"},

{type:"ios-refresh"},

{type:"ios-refresh-outline"},

{type:"ios-refresh-empty"},

{type:"ios-reload"},

{type:"ios-loop-strong"},

{type:"ios-loop"},

{type:"ios-bookmarks"},

{type:"ios-bookmarks-outline"},

{type:"ios-book"},

{type:"ios-book-outline"},

{type:"ios-flag"},

{type:"ios-flag-outline"},

{type:"ios-glasses"},

{type:"ios-glasses-outline"},

{type:"ios-browsers"},

{type:"ios-browsers-outline"},

{type:"ios-at"},

{type:"ios-at-outline"},

{type:"ios-pricetag"},

{type:"ios-pricetag-outline"},

{type:"ios-pricetags"},

{type:"ios-pricetags-outline"},

{type:"ios-cart"},

{type:"ios-cart-outline"},

{type:"ios-chatboxes"},

{type:"ios-chatboxes-outline"},

{type:"ios-chatbubble"},

{type:"ios-chatbubble-outline"},

{type:"ios-cog"},

{type:"ios-cog-outline"},

{type:"ios-gear"},

{type:"ios-gear-outline"},

{type:"ios-settings"},

{type:"ios-settings-strong"},

{type:"ios-toggle"},

{type:"ios-toggle-outline"},

{type:"ios-analytics"},

{type:"ios-analytics-outline"},

{type:"ios-pie"},

{type:"ios-pie-outline"},

{type:"ios-pulse"},

{type:"ios-pulse-strong"},

{type:"ios-filing"},

{type:"ios-filing-outline"},

{type:"ios-box"},

{type:"ios-box-outline"},

{type:"ios-compose"},

{type:"ios-compose-outline"},

{type:"ios-trash"},

{type:"ios-trash-outline"},

{type:"ios-copy"},

{type:"ios-copy-outline"},

{type:"ios-email"},

{type:"ios-email-outline"},

{type:"ios-undo"},

{type:"ios-undo-outline"},

{type:"ios-redo"},

{type:"ios-redo-outline"},

{type:"ios-paperplane"},

{type:"ios-paperplane-outline"},

{type:"ios-folder"},

{type:"ios-folder-outline"},

{type:"ios-paper"},

{type:"ios-paper-outline"},

{type:"ios-list"},

{type:"ios-list-outline"},

{type:"ios-world"},

{type:"ios-world-outline"},

{type:"ios-alarm"},

{type:"ios-alarm-outline"},

{type:"ios-speedometer"},

{type:"ios-speedometer-outline"},

{type:"ios-stopwatch"},

{type:"ios-stopwatch-outline"},

{type:"ios-timer"},

{type:"ios-timer-outline"},

{type:"ios-clock"},

{type:"ios-clock-outline"},

{type:"ios-time"},

{type:"ios-time-outline"},

{type:"ios-calendar"},

{type:"ios-calendar-outline"},

{type:"ios-photos"},

{type:"ios-photos-outline"},

{type:"ios-albums"},

{type:"ios-albums-outline"},

{type:"ios-camera"},

{type:"ios-camera-outline"},

{type:"ios-reverse-camera"},

{type:"ios-reverse-camera-outline"},

{type:"ios-eye"},

{type:"ios-eye-outline"},

{type:"ios-bolt"},

{type:"ios-bolt-outline"},

{type:"ios-color-wand"},

{type:"ios-color-wand-outline"},

{type:"ios-color-filter"},

{type:"ios-color-filter-outline"},

{type:"ios-grid-view"},

{type:"ios-grid-view-outline"},

{type:"ios-crop-strong"},

{type:"ios-crop"},

{type:"ios-barcode"},

{type:"ios-barcode-outline"},

{type:"ios-briefcase"},

{type:"ios-briefcase-outline"},

{type:"ios-medkit"},

{type:"ios-medkit-outline"},

{type:"ios-medical"},

{type:"ios-medical-outline"},

{type:"ios-infinite"},

{type:"ios-infinite-outline"},

{type:"ios-calculator"},

{type:"ios-calculator-outline"},

{type:"ios-keypad"},

{type:"ios-keypad-outline"},

{type:"ios-telephone"},

{type:"ios-telephone-outline"},

{type:"ios-drag"},

{type:"ios-location"},

{type:"ios-location-outline"},

{type:"ios-navigate"},

{type:"ios-navigate-outline"},

{type:"ios-locked"},

{type:"ios-locked-outline"},

{type:"ios-unlocked"},

{type:"ios-unlocked-outline"},

{type:"ios-monitor"},

{type:"ios-monitor-outline"},

{type:"ios-printer"},

{type:"ios-printer-outline"},

{type:"ios-game-controller-a"},

{type:"ios-game-controller-a-outline"},

{type:"ios-game-controller-b"},

{type:"ios-game-controller-b-outline"},

{type:"ios-americanfootball"},

{type:"ios-americanfootball-outline"},

{type:"ios-baseball"},

{type:"ios-baseball-outline"},

{type:"ios-basketball"},

{type:"ios-basketball-outline"},

{type:"ios-tennisball"},

{type:"ios-tennisball-outline"},

{type:"ios-football"},

{type:"ios-football-outline"},

{type:"ios-body"},

{type:"ios-body-outline"},

{type:"ios-person"},

{type:"ios-person-outline"},

{type:"ios-personadd"},

{type:"ios-personadd-outline"},

{type:"ios-people"},

{type:"ios-people-outline"},

{type:"ios-musical-notes"},

{type:"ios-musical-note"},

{type:"ios-bell"},

{type:"ios-bell-outline"},

{type:"ios-mic"},

{type:"ios-mic-outline"},

{type:"ios-mic-off"},

{type:"ios-volume-high"},

{type:"ios-volume-low"},

{type:"ios-play"},

{type:"ios-play-outline"},

{type:"ios-pause"},

{type:"ios-pause-outline"},

{type:"ios-recording"},

{type:"ios-recording-outline"},

{type:"ios-fastforward"},

{type:"ios-fastforward-outline"},

{type:"ios-rewind"},

{type:"ios-rewind-outline"},

{type:"ios-skipbackward"},

{type:"ios-skipbackward-outline"},

{type:"ios-skipforward"},

{type:"ios-skipforward-outline"},

{type:"ios-shuffle-strong"},

{type:"ios-shuffle"},

{type:"ios-videocam"},

{type:"ios-videocam-outline"},

{type:"ios-film"},

{type:"ios-film-outline"},

{type:"ios-flask"},

{type:"ios-flask-outline"},

{type:"ios-lightbulb"},

{type:"ios-lightbulb-outline"},

{type:"ios-wineglass"},

{type:"ios-wineglass-outline"},

{type:"ios-pint"},

{type:"ios-pint-outline"},

{type:"ios-nutrition"},

{type:"ios-nutrition-outline"},

{type:"ios-flower"},

{type:"ios-flower-outline"},

{type:"ios-rose"},

{type:"ios-rose-outline"},

{type:"ios-paw"},

{type:"ios-paw-outline"},

{type:"ios-flame"},

{type:"ios-flame-outline"},

{type:"ios-sunny"},

{type:"ios-sunny-outline"},

{type:"ios-partlysunny"},

{type:"ios-partlysunny-outline"},

{type:"ios-cloudy"},

{type:"ios-cloudy-outline"},

{type:"ios-rainy"},

{type:"ios-rainy-outline"},

{type:"ios-thunderstorm"},

{type:"ios-thunderstorm-outline"},

{type:"ios-snowy"},

{type:"ios-moon"},

{type:"ios-moon-outline"},

{type:"ios-cloudy-night"},

{type:"ios-cloudy-night-outline"},

{type:"android-arrow-up"},

{type:"android-arrow-forward"},

{type:"android-arrow-down"},

{type:"android-arrow-back"},

{type:"android-arrow-dropup"},

{type:"android-arrow-dropup-circle"},

{type:"android-arrow-dropright"},

{type:"android-arrow-dropright-circle"},

{type:"android-arrow-dropdown"},

{type:"android-arrow-dropdown-circle"},

{type:"android-arrow-dropleft"},

{type:"android-arrow-dropleft-circle"},

{type:"android-add"},

{type:"android-add-circle"},

{type:"android-remove"},

{type:"android-remove-circle"},

{type:"android-close"},

{type:"android-cancel"},

{type:"android-radio-button-off"},

{type:"android-radio-button-on"},

{type:"android-checkmark-circle"},

{type:"android-checkbox-outline-blank"},

{type:"android-checkbox-outline"},

{type:"android-checkbox-blank"},

{type:"android-checkbox"},

{type:"android-done"},

{type:"android-done-all"},

{type:"android-menu"},

{type:"android-more-horizontal"},

{type:"android-more-vertical"},

{type:"android-refresh"},

{type:"android-sync"},

{type:"android-wifi"},

{type:"android-call"},

{type:"android-apps"},

{type:"android-settings"},

{type:"android-options"},

{type:"android-funnel"},

{type:"android-search"},

{type:"android-home"},

{type:"android-cloud-outline"},

{type:"android-cloud"},

{type:"android-download"},

{type:"android-upload"},

{type:"android-cloud-done"},

{type:"android-cloud-circle"},

{type:"android-favorite-outline"},

{type:"android-favorite"},

{type:"android-star-outline"},

{type:"android-star-half"},

{type:"android-star"},

{type:"android-calendar"},

{type:"android-alarm-clock"},

{type:"android-time"},

{type:"android-stopwatch"},

{type:"android-watch"},

{type:"android-locate"},

{type:"android-navigate"},

{type:"android-pin"},

{type:"android-compass"},

{type:"android-map"},

{type:"android-walk"},

{type:"android-bicycle"},

{type:"android-car"},

{type:"android-bus"},

{type:"android-subway"},

{type:"android-train"},

{type:"android-boat"},

{type:"android-plane"},

{type:"android-restaurant"},

{type:"android-bar"},

{type:"android-cart"},

{type:"android-camera"},

{type:"android-image"},

{type:"android-film"},

{type:"android-color-palette"},

{type:"android-create"},

{type:"android-mail"},

{type:"android-drafts"},

{type:"android-send"},

{type:"android-archive"},

{type:"android-delete"},

{type:"android-attach"},

{type:"android-share"},

{type:"android-share-alt"},

{type:"android-bookmark"},

{type:"android-document"},

{type:"android-clipboard"},

{type:"android-list"},

{type:"android-folder-open"},

{type:"android-folder"},

{type:"android-print"},

{type:"android-open"},

{type:"android-exit"},

{type:"android-contract"},

{type:"android-expand"},

{type:"android-globe"},

{type:"android-chat"},

{type:"android-textsms"},

{type:"android-hangout"},

{type:"android-happy"},

{type:"android-sad"},

{type:"android-person"},

{type:"android-people"},

{type:"android-person-add"},

{type:"android-contact"},

{type:"android-contacts"},

{type:"android-playstore"},

{type:"android-lock"},

{type:"android-unlock"},

{type:"android-microphone"},

{type:"android-microphone-off"},

{type:"android-notifications-none"},

{type:"android-notifications"},

{type:"android-notifications-off"},

{type:"android-volume-mute"},

{type:"android-volume-down"},

{type:"android-volume-up"},

{type:"android-volume-off"},

{type:"android-hand"},

{type:"android-desktop"},

{type:"android-laptop"},

{type:"android-phone-portrait"},

{type:"android-phone-landscape"},

{type:"android-bulb"},

{type:"android-sunny"},

{type:"android-alert"},

{type:"android-warning"},

{type:"social-twitter"},

{type:"social-twitter-outline"},

{type:"social-facebook"},

{type:"social-facebook-outline"},

{type:"social-googleplus"},

{type:"social-googleplus-outline"},

{type:"social-google"},

{type:"social-google-outline"},

{type:"social-dribbble"},

{type:"social-dribbble-outline"},

{type:"social-octocat"},

{type:"social-github"},

{type:"social-github-outline"},

{type:"social-instagram"},

{type:"social-instagram-outline"},

{type:"social-whatsapp"},

{type:"social-whatsapp-outline"},

{type:"social-snapchat"},

{type:"social-snapchat-outline"},

{type:"social-foursquare"},

{type:"social-foursquare-outline"},

{type:"social-pinterest"},

{type:"social-pinterest-outline"},

{type:"social-rss"},

{type:"social-rss-outline"},

{type:"social-tumblr"},

{type:"social-tumblr-outline"},

{type:"social-wordpress"},

{type:"social-wordpress-outline"},

{type:"social-reddit"},

{type:"social-reddit-outline"},

{type:"social-hackernews"},

{type:"social-hackernews-outline"},

{type:"social-designernews"},

{type:"social-designernews-outline"},

{type:"social-yahoo"},

{type:"social-yahoo-outline"},

{type:"social-buffer"},

{type:"social-buffer-outline"},

{type:"social-skype"},

{type:"social-skype-outline"},

{type:"social-linkedin"},

{type:"social-linkedin-outline"},

{type:"social-vimeo"},

{type:"social-vimeo-outline"},

{type:"social-twitch"},

{type:"social-twitch-outline"},

{type:"social-youtube"},

{type:"social-youtube-outline"},

{type:"social-dropbox"},

{type:"social-dropbox-outline"},

{type:"social-apple"},

{type:"social-apple-outline"},

{type:"social-android"},

{type:"social-android-outline"},

{type:"social-windows"},

{type:"social-windows-outline"},

{type:"social-html5"},

{type:"social-html5-outline"},

{type:"social-css3"},

{type:"social-css3-outline"},

{type:"social-javascript"},

{type:"social-javascript-outline"},

{type:"social-angular"},

{type:"social-angular-outline"},

{type:"social-nodejs"},

{type:"social-sass"},

{type:"social-python"},

{type:"social-chrome"},

{type:"social-chrome-outline"},

{type:"social-codepen"},

{type:"social-codepen-outline"},

{type:"social-markdown"},

{type:"social-tux"},

{type:"social-freebsd-devil"},

{type:"social-usd"},

{type:"social-usd-outline"},

{type:"social-bitcoin"},

{type:"social-bitcoin-outline"},

{type:"social-yen"},

{type:"social-yen-outline"},

{type:"social-euro"},

{type:"social-euro-outline"}
         ] 
      }
  }
}
</script>